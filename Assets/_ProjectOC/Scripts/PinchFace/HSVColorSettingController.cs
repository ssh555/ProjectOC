...
namespace ML.Engine.UI
{
    public class HSVColorSettingController : MonoBehaviour
    {
        ...
        private void BindColorAction()
        {
            for (int i = 0; i < 3; i++)
            {
                sliders[i*2].slider.onValueChanged.AddListener((_value)=>
                {
                ...
                sliders[i*2+1].slider.onValueChanged.AddListener((_value)=>
                {
        ...
        public Color ReadColor(bool left = true)
        {
            ...
            if (left)
            {
                Vector3 colorVec = new Vector3(sliders[0].value, sliders[2].value, sliders[4].value) / 255f;
                res = new Color(colorVec.x, colorVec.y, colorVec.z, 1.0f);
            }
            else
            {
                Vector3 colorVec = new Vector3(sliders[1].value, sliders[3].value, sliders[5].value) / 255f;
        ...
        public void SetColorValue(Color _colorVaue, bool left = true)
        {
            if (left)
            {
                ...
                sliders[2].SetValueWithoutNotify(_values.y);
                sliders[4].SetValueWithoutNotify(_values.z);
            }
            else
            {
                float _h, _s, _v;
                ...
                sliders[1].SetValueWithoutNotify(_values.x);
                sliders[3].SetValueWithoutNotify(_values.y);
