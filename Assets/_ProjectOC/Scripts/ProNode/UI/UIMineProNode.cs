...
namespace ProjectOC.ProNodeNS.UI
{
    public class UIMineProNode : ML.Engine.UI.UIBasePanel<ProNodePanel>
    {
        ...
        private const string strMine = "Mine";
        private const string strWorkerMaleIcon = "WorkerMaleIcon";
        private const string strWorkerFemalIcon = "WorkerFemalIcon";
        ...
        private const string strKT_ChangeMine = "KT_ChangeMine";
        ...
        private const string strIconMine = "IconMine";
        ...
        private const string strPrefab_Mine_UI_SelectMineralSourcesPanel = "Prefab_Mine_UIPanel/Prefab_Mine_UI_SelectMineralSourcesPanel.prefab";
        private const string strPrefab_Mine_UI_SmallMapPanel = "Prefab_Mine_UIPanel/Prefab_Mine_UI_SmallMapPanel.prefab";
        #endregion
        ...
        protected override void InitBtnInfo()
        {
            ...
            ProductBtnList = new ML.Engine.UI.UIBtnList(transform.Find(strProNode).Find(strProduct).Find(strViewport).GetComponentInChildren<ML.Engine.UI.UIBtnListInitor>());
        ...
        protected override void Start()
        {
            ...
            #endregion
            #region ProNode
            ProNode_Mine = transform.Find(strProNode).Find(strMine);
        ...
        protected override void Enter()
        {
            ...
            tempSprite.Add(strWorkerMaleIcon, ManagerNS.LocalGameManager.Instance.WorkerManager.GetSprite(strTex2D_Worker_UI_GenderMale));
            tempSprite.Add(strWorkerFemalIcon, ManagerNS.LocalGameManager.Instance.WorkerManager.GetSprite(strTex2D_Worker_UI_GenderFemale));
            base.Enter();
        }
        ...
        private IEnumerator PushSmallMapPanel(UISelectMineralSourcesPanel selectMineralSourcesPanel)
        {
            ...
            GameManager.Instance.ABResourceManager.InstantiateAsync(strPrefab_Mine_UI_SmallMapPanel).Completed += (handle) =>
            {
        ...
        private void SetUIActive()
        {
            ...
            if (CurMode == Mode.ProNode)
            {
                BotKeyTips.Find(strKT_ChangeMine).gameObject.SetActive(CurProNodeMode == ProNodeSelectMode.Mine);
                ...
                BotKeyTips.Find(strKT_Remove1).gameObject.SetActive(CurProNodeMode == ProNodeSelectMode.Product);
                BotKeyTips.Find(strKT_Remove10).gameObject.SetActive(CurProNodeMode == ProNodeSelectMode.Product);
                BotKeyTips.Find(strKT_Return).gameObject.SetActive(CurProNodeMode == ProNodeSelectMode.Mine);
        ...
        public override void Refresh()
        {
            ...
            if (CurMode == Mode.ProNode)
            {
                ...
                #region Mine
                ProNode_Mine.Find(strSelected).gameObject.SetActive(CurProNodeMode == ProNodeSelectMode.Mine);
                bool hasMine = ProNode.HasMine;
                ProNode_Mine.Find(strIcon).gameObject.SetActive(!hasMine);
                ProNode_Mine.Find(strIconMine).gameObject.SetActive(hasMine);
                ProNode_Mine.Find(strName).gameObject.SetActive(hasMine && ProNode.IsOnProduce);
                ProNode_Mine.Find(strName).GetComponent<TMPro.TextMeshProUGUI>().text = PanelTextContent.textChangeMine;
                #endregion
                ...
                for (int i = 0; i < mineCnt; i++)
                {
                    ...
                    mine.Find(strIcon).gameObject.SetActive(true);
                    ...
                    mine.Find(strIcon).GetComponent<Image>().sprite = tempSprite[productID];
                    mine.Find(strBack).gameObject.SetActive(false);
                    mine.Find(strBack2).GetComponent<Image>().color = stackAll >= stackMax ? new Color(113/255f, 182/255f, 4/255f) : Color.black;
                    mine.Find(strAmount).GetComponent<TMPro.TextMeshProUGUI>().text = ProNode.DataContainer.GetAmount(i, DataNS.DataOpType.Storage).ToString();
                    mine.Find(strName).GetComponent<TMPro.TextMeshProUGUI>().text = ML.Engine.InventorySystem.ItemManager.Instance.GetItemName(productID);
                }
                for (int i = mineCnt; i < ProductBtnList.BtnCnt; i++)
                {
                    Transform empty = ProductBtnList.GetBtn(i).transform;
                    empty.Find(strIcon).gameObject.SetActive(false);
                    empty.Find(strBack).gameObject.SetActive(true);
                    empty.Find(strAmount).GetComponent<TMPro.TextMeshProUGUI>().text = str0;
                    empty.Find(strName).GetComponent<TMPro.TextMeshProUGUI>().text = str;
                }
                if (!ProNode.HasMine || ProNode.State != ProNodeState.Production)
                {
                    ProNode_Mine.Find(strMask).GetComponent<Image>().fillAmount = 0;
                }
                #endregion
                ...
                if (hasWorker)
                {
                    ...
                    ProNode_Worker.Find(strGender).GetComponent<Image>().sprite = Worker.Gender == Gender.Male ? tempSprite[strWorkerMaleIcon] : tempSprite[strWorkerFemalIcon];
                }
                #endregion
            ...
            else if (CurMode == Mode.ChangeWorker)
            {
                for (int i = 0; i < Workers.Count; ++i)
                {
                    ...
                    // Gender
                    item.Find(strGender).GetComponent<Image>().sprite = worker.Gender == Gender.Male ? tempSprite[strWorkerMaleIcon] : tempSprite[strWorkerFemalIcon];
                    // Eff
                    ...
                    item.Find(strEff).GetComponent<TMPro.TextMeshProUGUI>().text = strAddSignal + worker.GetEff(ProNode.ExpType).ToString() + strModSignal;
        ...
        {
            ProNode_Mine.Find(strMask).GetComponent<Image>().fillAmount = 1 - (float)(time / ProNode.GetTimeCost());
        }
        ...
        public void RefreshDynamic()
        {
            ...
            ProNode_Mine.Find(strName).gameObject.SetActive(ProNode.HasMine && ProNode.IsOnProduce);
            int mineCnt = ProNode.MineDataItemIDs?.Count ?? 0;
            for (int i = 0; i < mineCnt; i++)
            {
                Transform mine = ProductBtnList.GetBtn(i).transform;
            ...
            if (ProNode.HaveWorker)
            {
                ...
                ProNode_Eff.Find(strEffPrefix).GetComponent<TMPro.TextMeshProUGUI>().text = PanelTextContent.textEff;
                ProNode_Eff.Find(strEff).GetComponent<TMPro.TextMeshProUGUI>().text = strAddSignal + (ProNode?.GetEff().ToString() ?? str) + strModSignal;
                string buildID = ML.Engine.BuildingSystem.BuildingManager.Instance.GetID(ProNode.WorldProNode.Classification.ToString());
                if (!tempSprite.ContainsKey(buildID))
                {
                    tempSprite[buildID] = ML.Engine.BuildingSystem.BuildingManager.Instance.GetBuildIcon(buildID);
